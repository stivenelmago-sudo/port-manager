/**
 * PortPilot - Translation messages
 * 6 idiomas: en, es, zh, hi, ar, ja
 */

const MESSAGES = {
  en: {
    ext: {
      description: "PortPilot",
    },
    cmd: {
      show: "PortPilot: Show Listening Ports",
      checkPort: "PortPilot: Check Port Availability",
      killPort: "PortPilot: Kill Port",
    },
    view: {
      title: "Ports",
    },
    noPorts: "No listening ports",
    quickPickPlaceholder: "Listening ports — select to KILL",
    killConfirm: (port, process) => `Kill port :${port} (${process})?`,
    killButton: "KILL",
    inputPromptCheck: "Enter port number to check",
    inputPlaceholderCheck: "e.g. 3000",
    portFree: (port) => `Port :${port} is free`,
    portUsed: (port, detail) => `Port :${port} is in use${detail}`,
    inputPromptKill: "Enter port number(s) to close (comma-separated)",
    inputPlaceholderKill: "e.g. 3000 or 3000,8080,5432",
    noMatchingPorts: "No matching ports found",
    bulkKillConfirm: (n, desc) => `Kill ${n} port(s): ${desc}`,
    bulkKillResult: (ok, fail) => `Done: success ${ok} / failed ${fail}`,
    killed: (port) => `Port :${port} was killed`,
    killFailed: (msg) => `Kill failed: ${msg}`,
    validateRange: (min, max) => `Enter a value between ${min}-${max}`,
    setLanguage: {
      prompt: "Select language",
      current: "current",
      changed: (lang) => `Language: ${lang} (reload window)`,
    },
    webview: {
      searchPlaceholder: "Search by port / process name...",
      refresh: "Refresh",
      bulkKill: (n) => `KILL Selected (${n})`,
      run: "Run",
      statsUsed: (n) => `Used ${n}`,
      statsFree: (n) => `Free ${n}`,
      statsTotal: (n) => `Total ${n}`,
      empty: "No matching ports",
      colPort: "Port",
      colState: "State",
      colProcess: "Process",
      colPid: "PID",
      colAncestry: "Ancestry",
      colAction: "Action",
      stateListen: "LISTEN",
      stateFree: "FREE",
      kill: "KILL",
      confirm: "Confirm",
      cancel: "Cancel",
      toastKilled: (port) => `:${port} was killed`,
      toastKillFailed: (msg) => `Kill failed: ${msg}`,
      bulkKilledLabel: (n) => `${n} ports`,
      langMenu: "🌐 Language",
      statsAncestry: "with ancestry",
      ancestryNone: "—",
      ancestryLoading: "loading…",
      witrMissing: "Process ancestry unavailable",
      witrPermission: "Run VS Code as Admin/sudo for full ancestry",
      tabPorts: "Ports",
      tabProcesses: "Processes",
      autoRefreshOn: "Auto-refresh ON",
      autoRefreshOff: "Auto-refresh OFF",
      detailsTitle: "Process Details",
      detailsTitleContainer: "Container Details",
      detailsTitleLock: "File Lock Details",
      detailsAncestry: "Ancestry",
      detailsCwd: "Working Dir",
      detailsEnv: "Environment",
      detailsSockets: "Sockets",
      detailsSource: "Source",
      detailsStarted: "Started",
      detailsCommand: "Command",
      detailsUser: "User",
      detailsNoData: "Select a process to view details",
      detailsLoading: "Loading process details…",
      detailsNotAvailable: "Process details not available",
      emptyProcesses: "No processes found",
      colCpu: "CPU",
      colMemory: "Memory",
      colCommand: "Command",
      tabContainers: "Containers",
      emptyContainers: "No containers found",
      colRuntime: "Runtime",
      colImage: "Image",
      colStatus: "Status",
      colAction2: "Actions",
      actionStop: "Stop",
      actionRestart: "Restart",
      actionStart: "Start",
      actionPause: "Pause",
      actionLogs: "Logs",
      actionInspect: "Inspect",
      tabLocks: "Locks",
      emptyLocks: "No file locks found",
      colFd: "FD",
      colPath: "Path",
      colInode: "Inode",
      locksAllOpen: "Show all open files",
      actionTerminate: "Terminate",
      actionResume: "Resume",
      actionRenice: "Renice",
      detailsClose: "Close",
      pidPrefix: "pid ",
      containerImage: "Image",
      containerState: "State",
      containerCommand: "Command",
      containerMounts: "Mounts",
      containerNetworks: "Networks",
      containerEnv: "Env (first 30)",
      actionCopy: "Copy",
      copied: "Copied",
      copyPort: "Copy port",
      copyPid: "Copy PID",
      copyCommand: "Copy command",
      copyPath: "Copy path",
      openInBrowser: "Open in browser",
      actionOpen: "Open",
      niceValue: "Nice value (-20 to 19)",
      // MCP tools tab
      tabTools: "Tools",
      mcpMasterLabel: "MCP server",
      mcpMasterOn: "Enabled",
      mcpMasterOff: "Disabled",
      mcpStatusRunning: "Active",
      mcpStatusStopped: "Stopped",
      mcpAutoconfigHint: "Auto-registered with VS Code, Cursor, Copilot, etc.",
      mcpCategoryRead: "Read",
      mcpCategoryWrite: "Write",
      mcpCategorySystem: "System",
      mcpDestructiveFlag: "destructive",
      mcpConfigPathHint: "Runtime config file",
      mcpServerVersionLabel: "Server version",
      mcpSelectAll: "Enable all",
      mcpDeselectAll: "Disable all",
    },
  },
  es: {
    ext: {
      description: "PortPilot",
    },
    cmd: {
      show: "PortPilot: Mostrar puertos en escucha",
      checkPort: "PortPilot: Verificar disponibilidad de puerto",
      killPort: "PortPilot: Cerrar puerto",
    },
    view: {
      title: "Puertos",
    },
    noPorts: "No hay puertos en escucha",
    quickPickPlaceholder: "Puertos en escucha — selecciona para CERRAR",
    killConfirm: (port, process) => `¿Cerrar el puerto :${port} (${process})?`,
    killButton: "CERRAR",
    inputPromptCheck: "Ingresa el número de puerto a verificar",
    inputPlaceholderCheck: "ej. 3000",
    portFree: (port) => `El puerto :${port} está libre`,
    portUsed: (port, detail) => `El puerto :${port} está en uso${detail}`,
    inputPromptKill: "Ingresa el(los) puerto(s) a cerrar (separados por coma)",
    inputPlaceholderKill: "ej. 3000 o 3000,8080,5432",
    noMatchingPorts: "No se encontraron puertos coincidentes",
    bulkKillConfirm: (n, desc) => `Cerrar ${n} puerto(s): ${desc}`,
    bulkKillResult: (ok, fail) => `Listo: éxito ${ok} / fallo ${fail}`,
    killed: (port) => `Puerto :${port} cerrado`,
    killFailed: (msg) => `Error al cerrar: ${msg}`,
    validateRange: (min, max) => `Ingresa un valor entre ${min}-${max}`,
    setLanguage: {
      prompt: "Seleccionar idioma",
      current: "actual",
      changed: (lang) => `Idioma: ${lang} (recargar ventana)`,
    },
    webview: {
      searchPlaceholder: "Buscar por puerto / proceso...",
      refresh: "Actualizar",
      bulkKill: (n) => `CERRAR selección (${n})`,
      run: "Ejecutar",
      statsUsed: (n) => `En uso ${n}`,
      statsFree: (n) => `Libres ${n}`,
      statsTotal: (n) => `Total ${n}`,
      empty: "No hay puertos coincidentes",
      colPort: "Puerto",
      colState: "Estado",
      colProcess: "Proceso",
      colPid: "PID",
      colAncestry: "Ancestros",
      colAction: "Acción",
      stateListen: "ESCUCHA",
      stateFree: "LIBRE",
      kill: "CERRAR",
      confirm: "Confirmar",
      cancel: "Cancelar",
      toastKilled: (port) => `Puerto :${port} cerrado`,
      toastKillFailed: (msg) => `Error al cerrar: ${msg}`,
      bulkKilledLabel: (n) => `${n} puertos`,
      langMenu: "🌐 Idioma",
      statsAncestry: "con ancestros",
      ancestryNone: "—",
      ancestryLoading: "cargando…",
      witrMissing: "Ancestros no disponibles",
      witrPermission: "Ejecuta VS Code como Admin/sudo para ancestros completos",
      tabPorts: "Puertos",
      tabProcesses: "Procesos",
      autoRefreshOn: "Auto-refresco ON",
      autoRefreshOff: "Auto-refresco OFF",
      detailsTitle: "Detalles del proceso",
      detailsTitleContainer: "Detalles del contenedor",
      detailsTitleLock: "Detalles del bloqueo de archivo",
      detailsAncestry: "Ancestros",
      detailsCwd: "Dir. de trabajo",
      detailsEnv: "Entorno",
      detailsSockets: "Sockets",
      detailsSource: "Origen",
      detailsStarted: "Iniciado",
      detailsCommand: "Comando",
      detailsUser: "Usuario",
      detailsNoData: "Selecciona un proceso para ver detalles",
      detailsLoading: "Cargando detalles…",
      detailsNotAvailable: "Detalles no disponibles",
      emptyProcesses: "Sin procesos",
      colCpu: "CPU",
      colMemory: "Memoria",
      colCommand: "Comando",
      tabContainers: "Contenedores",
      emptyContainers: "Sin contenedores",
      colRuntime: "Runtime",
      colImage: "Imagen",
      colStatus: "Estado",
      colAction2: "Acciones",
      actionStop: "Detener",
      actionRestart: "Reiniciar",
      actionStart: "Iniciar",
      actionPause: "Pausar",
      actionLogs: "Logs",
      actionInspect: "Inspeccionar",
      tabLocks: "Bloqueos",
      emptyLocks: "Sin bloqueos de archivo",
      colFd: "FD",
      colPath: "Ruta",
      colInode: "Inodo",
      locksAllOpen: "Mostrar todos los archivos abiertos",
      actionTerminate: "Terminar",
      actionResume: "Reanudar",
      actionRenice: "Renice",
      detailsClose: "Cerrar",
      pidPrefix: "pid ",
      containerImage: "Imagen",
      containerState: "Estado",
      containerCommand: "Comando",
      containerMounts: "Montajes",
      containerNetworks: "Redes",
      containerEnv: "Entorno (primeros 30)",
      actionCopy: "Copiar",
      copied: "Copiado",
      copyPort: "Copiar puerto",
      copyPid: "Copiar PID",
      copyCommand: "Copiar comando",
      copyPath: "Copiar ruta",
      openInBrowser: "Abrir en navegador",
      actionOpen: "Abrir",
      niceValue: "Valor nice (-20 a 19)",
      tabTools: "Tools",
      mcpMasterLabel: "Servidor MCP",
      mcpMasterOn: "Activado",
      mcpMasterOff: "Desactivado",
      mcpStatusRunning: "Activo",
      mcpStatusStopped: "Detenido",
      mcpAutoconfigHint: "Auto-registrado con VS Code, Cursor, Copilot, etc.",
      mcpCategoryRead: "Lectura",
      mcpCategoryWrite: "Escritura",
      mcpCategorySystem: "Sistema",
      mcpDestructiveFlag: "destructivo",
      mcpConfigPathHint: "Archivo de configuración",
      mcpServerVersionLabel: "Versión del servidor",
      mcpSelectAll: "Activar todos",
      mcpDeselectAll: "Desactivar todos",
    },
  },
  zh: {
    ext: {
      description: "PortPilot",
    },
    cmd: {
      show: "PortPilot: 显示监听端口",
      checkPort: "PortPilot: 检查端口可用性",
      killPort: "PortPilot: 关闭端口",
    },
    view: {
      title: "端口",
    },
    noPorts: "没有监听的端口",
    quickPickPlaceholder: "监听的端口 — 选择以关闭",
    killConfirm: (port, process) => `关闭端口 :${port} (${process})?`,
    killButton: "关闭",
    inputPromptCheck: "输入要检查的端口号",
    inputPlaceholderCheck: "例如 3000",
    portFree: (port) => `端口 :${port} 是空闲的`,
    portUsed: (port, detail) => `端口 :${port} 正在使用${detail}`,
    inputPromptKill: "输入要关闭的端口号(多个用逗号分隔)",
    inputPlaceholderKill: "例如 3000 或 3000,8080,5432",
    noMatchingPorts: "未找到匹配的端口",
    bulkKillConfirm: (n, desc) => `关闭 ${n} 个端口: ${desc}`,
    bulkKillResult: (ok, fail) => `完成: 成功 ${ok} / 失败 ${fail}`,
    killed: (port) => `端口 :${port} 已关闭`,
    killFailed: (msg) => `关闭失败: ${msg}`,
    validateRange: (min, max) => `请输入 ${min}-${max} 之间的值`,
    setLanguage: {
      prompt: "选择语言",
      current: "当前",
      changed: (lang) => `语言: ${lang} (重新加载窗口)`,
    },
    webview: {
      searchPlaceholder: "按端口/进程搜索...",
      refresh: "刷新",
      bulkKill: (n) => `关闭所选 (${n})`,
      run: "执行",
      statsUsed: (n) => `使用 ${n}`,
      statsFree: (n) => `空闲 ${n}`,
      statsTotal: (n) => `总计 ${n}`,
      empty: "没有匹配的端口",
      colPort: "端口",
      colState: "状态",
      colProcess: "进程",
      colPid: "PID",
      colAncestry: "来源链",
      colAction: "操作",
      stateListen: "监听",
      stateFree: "空闲",
      kill: "关闭",
      confirm: "确认",
      cancel: "取消",
      toastKilled: (port) => `端口 :${port} 已关闭`,
      toastKillFailed: (msg) => `关闭失败: ${msg}`,
      bulkKilledLabel: (n) => `${n} 个端口`,
      langMenu: "🌐 语言",
      statsAncestry: "有来源链",
      ancestryNone: "—",
      ancestryLoading: "加载中…",
      witrMissing: "来源链不可用",
      witrPermission: "以管理员/sudo 启动 VS Code 以获得完整来源链",
      tabPorts: "端口",
      tabProcesses: "进程",
      autoRefreshOn: "自动刷新 开",
      autoRefreshOff: "自动刷新 关",
      detailsTitle: "进程详情",
      detailsTitleContainer: "容器详情",
      detailsTitleLock: "文件锁详情",
      detailsAncestry: "来源链",
      detailsCwd: "工作目录",
      detailsEnv: "环境变量",
      detailsSockets: "套接字",
      detailsSource: "来源",
      detailsStarted: "启动时间",
      detailsCommand: "命令行",
      detailsUser: "用户",
      detailsNoData: "选择进程以查看详情",
      detailsLoading: "加载中…",
      detailsNotAvailable: "详情不可用",
      emptyProcesses: "无进程",
      colCpu: "CPU",
      colMemory: "内存",
      colCommand: "命令行",
      tabContainers: "容器",
      emptyContainers: "无容器",
      colRuntime: "运行时",
      colImage: "镜像",
      colStatus: "状态",
      colAction2: "操作",
      actionStop: "停止",
      actionRestart: "重启",
      actionStart: "启动",
      actionPause: "暂停",
      actionLogs: "日志",
      actionInspect: "检查",
      tabLocks: "文件锁",
      emptyLocks: "无文件锁",
      colFd: "FD",
      colPath: "路径",
      colInode: "节点",
      locksAllOpen: "显示所有打开的文件",
      actionTerminate: "终止",
      actionResume: "继续",
      actionRenice: "调整优先级",
      detailsClose: "关闭",
      pidPrefix: "pid ",
      containerImage: "镜像",
      containerState: "状态",
      containerCommand: "命令",
      containerMounts: "挂载",
      containerNetworks: "网络",
      containerEnv: "环境变量 (前30)",
      actionCopy: "复制",
      copied: "已复制",
      copyPort: "复制端口",
      copyPid: "复制 PID",
      copyCommand: "复制命令",
      copyPath: "复制路径",
      openInBrowser: "在浏览器中打开",
      actionOpen: "打开",
      niceValue: "优先级值 (-20 到 19)",
      tabTools: "工具",
      mcpMasterLabel: "MCP 服务器",
      mcpMasterOn: "已启用",
      mcpMasterOff: "已禁用",
      mcpStatusRunning: "运行中",
      mcpStatusStopped: "已停止",
      mcpAutoconfigHint: "已自动注册到 VS Code、Cursor、Copilot 等",
      mcpCategoryRead: "读取",
      mcpCategoryWrite: "写入",
      mcpCategorySystem: "系统",
      mcpDestructiveFlag: "危险",
      mcpConfigPathHint: "配置文件",
      mcpServerVersionLabel: "服务器版本",
      mcpSelectAll: "全部启用",
      mcpDeselectAll: "全部禁用",
    },
  },
  hi: {
    ext: {
      description: "PortPilot",
    },
    cmd: {
      show: "PortPilot: सुनने वाले पोर्ट दिखाएँ",
      checkPort: "PortPilot: पोर्ट उपलब्धता जाँचें",
      killPort: "PortPilot: पोर्ट बंद करें",
    },
    view: {
      title: "पोर्ट्स",
    },
    noPorts: "कोई सुनने वाला पोर्ट नहीं",
    quickPickPlaceholder: "सुनने वाले पोर्ट — बंद करने के लिए चुनें",
    killConfirm: (port, process) => `पोर्ट :${port} (${process}) बंद करें?`,
    killButton: "बंद करें",
    inputPromptCheck: "जाँचने के लिए पोर्ट नंबर दर्ज करें",
    inputPlaceholderCheck: "उदा. 3000",
    portFree: (port) => `पोर्ट :${port} खाली है`,
    portUsed: (port, detail) => `पोर्ट :${port} उपयोग में है${detail}`,
    inputPromptKill: "बंद करने के लिए पोर्ट नंबर दर्ज करें (अल्पविराम से अलग)",
    inputPlaceholderKill: "उदा. 3000 या 3000,8080,5432",
    noMatchingPorts: "कोई मेल खाने वाला पोर्ट नहीं मिला",
    bulkKillConfirm: (n, desc) => `${n} पोर्ट बंद करें: ${desc}`,
    bulkKillResult: (ok, fail) => `पूर्ण: सफल ${ok} / असफल ${fail}`,
    killed: (port) => `पोर्ट :${port} बंद किया गया`,
    killFailed: (msg) => `बंद करने में विफल: ${msg}`,
    validateRange: (min, max) => `${min}-${max} के बीच मान दर्ज करें`,
    setLanguage: {
      prompt: "भाषा चुनें",
      current: "वर्तमान",
      changed: (lang) => `भाषा: ${lang} (विंडो पुनः लोड करें)`,
    },
    webview: {
      searchPlaceholder: "पोर्ट / प्रक्रिया से खोजें...",
      refresh: "ताज़ा करें",
      bulkKill: (n) => `चयनित बंद करें (${n})`,
      run: "चलाएँ",
      statsUsed: (n) => `उपयोग ${n}`,
      statsFree: (n) => `खाली ${n}`,
      statsTotal: (n) => `कुल ${n}`,
      empty: "कोई मेल खाने वाला पोर्ट नहीं",
      colPort: "पोर्ट",
      colState: "स्थिति",
      colProcess: "प्रक्रिया",
      colPid: "PID",
      colAncestry: "वंश",
      colAction: "क्रिया",
      stateListen: "सुन रहा",
      stateFree: "खाली",
      kill: "बंद",
      confirm: "पुष्टि करें",
      cancel: "रद्द करें",
      toastKilled: (port) => `पोर्ट :${port} बंद किया गया`,
      toastKillFailed: (msg) => `बंद करने में विफल: ${msg}`,
      bulkKilledLabel: (n) => `${n} पोर्ट`,
      langMenu: "🌐 भाषा",
      statsAncestry: "वंश के साथ",
      ancestryNone: "—",
      ancestryLoading: "लोड हो रहा…",
      witrMissing: "वंश उपलब्ध नहीं",
      witrPermission: "पूर्ण वंश के लिए VS Code को Admin/sudo से चलाएँ",
      tabPorts: "पोर्ट्स",
      tabProcesses: "प्रक्रियाएँ",
      autoRefreshOn: "ऑटो-रिफ्रेश ON",
      autoRefreshOff: "ऑटो-रिफ्रेश OFF",
      detailsTitle: "प्रक्रिया विवरण",
      detailsTitleContainer: "कंटेनर विवरण",
      detailsTitleLock: "फ़ाइल लॉक विवरण",
      detailsAncestry: "वंश",
      detailsCwd: "कार्य निर्देशिका",
      detailsEnv: "पर्यावरण",
      detailsSockets: "सॉकेट्स",
      detailsSource: "स्रोत",
      detailsStarted: "शुरू हुआ",
      detailsCommand: "कमांड",
      detailsUser: "उपयोगकर्ता",
      detailsNoData: "विवरण देखने के लिए प्रक्रिया चुनें",
      detailsLoading: "लोड हो रहा है…",
      detailsNotAvailable: "विवरण उपलब्ध नहीं",
      emptyProcesses: "कोई प्रक्रिया नहीं",
      colCpu: "CPU",
      colMemory: "मेमोरी",
      colCommand: "कमांड",
      tabContainers: "कंटेनर",
      emptyContainers: "कोई कंटेनर नहीं",
      colRuntime: "रनटाइम",
      colImage: "इमेज",
      colStatus: "स्थिति",
      colAction2: "क्रियाएँ",
      actionStop: "रोकें",
      actionRestart: "पुनरारंभ",
      actionStart: "शुरू",
      actionPause: "रोकें",
      actionLogs: "लॉग्स",
      actionInspect: "जाँचें",
      tabLocks: "ताले",
      emptyLocks: "कोई फ़ाइल ताला नहीं",
      colFd: "FD",
      colPath: "पथ",
      colInode: "इनोड",
      locksAllOpen: "सभी खुली फ़ाइलें दिखाएँ",
      actionTerminate: "समाप्त",
      actionResume: "पुनः आरंभ",
      actionRenice: "प्राथमिकता बदलें",
      detailsClose: "बंद करें",
      pidPrefix: "pid ",
      containerImage: "इमेज",
      containerState: "स्थिति",
      containerCommand: "कमांड",
      containerMounts: "माउंट",
      containerNetworks: "नेटवर्क",
      containerEnv: "पर्यावरण (पहले 30)",
      actionCopy: "कॉपी करें",
      copied: "कॉपी हो गया",
      copyPort: "पोर्ट कॉपी करें",
      copyPid: "PID कॉपी करें",
      copyCommand: "कमांड कॉपी करें",
      copyPath: "पथ कॉपी करें",
      openInBrowser: "ब्राउज़र में खोलें",
      actionOpen: "खोलें",
      niceValue: "Nice मान (-20 से 19)",
      tabTools: "उपकरण",
      mcpMasterLabel: "MCP सर्वर",
      mcpMasterOn: "सक्षम",
      mcpMasterOff: "अक्षम",
      mcpStatusRunning: "सक्रिय",
      mcpStatusStopped: "रुका हुआ",
      mcpAutoconfigHint: "VS Code, Cursor, Copilot आदि के साथ स्वतः पंजीकृत",
      mcpCategoryRead: "पढ़ें",
      mcpCategoryWrite: "लिखें",
      mcpCategorySystem: "सिस्टम",
      mcpDestructiveFlag: "विनाशकारी",
      mcpConfigPathHint: "कॉन्फ़िग फ़ाइल",
      mcpServerVersionLabel: "सर्वर संस्करण",
      mcpSelectAll: "सभी सक्षम करें",
      mcpDeselectAll: "सभी अक्षम करें",
    },
  },
  ar: {
    ext: {
      description: "PortPilot",
    },
    cmd: {
      show: "PortPilot: عرض المنافذ المستمعة",
      checkPort: "PortPilot: التحقق من توفر المنفذ",
      killPort: "PortPilot: إغلاق المنفذ",
    },
    view: {
      title: "المنافذ",
    },
    noPorts: "لا توجد منافذ مستمعة",
    quickPickPlaceholder: "المنافذ المستمعة — اختر للإغلاق",
    killConfirm: (port, process) => `إغلاق المنفذ :${port} (${process})؟`,
    killButton: "إغلاق",
    inputPromptCheck: "أدخل رقم المنفذ للتحقق",
    inputPlaceholderCheck: "مثال: 3000",
    portFree: (port) => `المنفذ :${port} متاح`,
    portUsed: (port, detail) => `المنفذ :${port} قيد الاستخدام${detail}`,
    inputPromptKill: "أدخل أرقام المنافذ للإغلاق (مفصولة بفواصل)",
    inputPlaceholderKill: "مثال: 3000 أو 3000,8080,5432",
    noMatchingPorts: "لم يتم العثور على منافذ مطابقة",
    bulkKillConfirm: (n, desc) => `إغلاق ${n} منفذ(ات): ${desc}`,
    bulkKillResult: (ok, fail) => `تم: نجاح ${ok} / فشل ${fail}`,
    killed: (port) => `تم إغلاق المنفذ :${port}`,
    killFailed: (msg) => `فشل الإغلاق: ${msg}`,
    validateRange: (min, max) => `أدخل قيمة بين ${min}-${max}`,
    setLanguage: {
      prompt: "اختر اللغة",
      current: "الحالي",
      changed: (lang) => `اللغة: ${lang} (أعد تحميل النافذة)`,
    },
    webview: {
      searchPlaceholder: "بحث عن منفذ / عملية...",
      refresh: "تحديث",
      bulkKill: (n) => `إغلاق المحدد (${n})`,
      run: "تشغيل",
      statsUsed: (n) => `قيد الاستخدام ${n}`,
      statsFree: (n) => `متاح ${n}`,
      statsTotal: (n) => `المجموع ${n}`,
      empty: "لا توجد منافذ مطابقة",
      colPort: "المنفذ",
      colState: "الحالة",
      colProcess: "العملية",
      colPid: "PID",
      colAncestry: "السلسلة",
      colAction: "إجراء",
      stateListen: "استماع",
      stateFree: "متاح",
      kill: "إغلاق",
      confirm: "تأكيد",
      cancel: "إلغاء",
      toastKilled: (port) => `تم إغلاق المنفذ :${port}`,
      toastKillFailed: (msg) => `فشل الإغلاق: ${msg}`,
      bulkKilledLabel: (n) => `${n} منافذ`,
      langMenu: "🌐 اللغة",
      statsAncestry: "بسلسلة",
      ancestryNone: "—",
      ancestryLoading: "جارٍ التحميل…",
      witrMissing: "السلسلة غير متاحة",
      witrPermission: "شغّل VS Code كمسؤول/sudo للحصول على السلسلة الكاملة",
      tabPorts: "المنافذ",
      tabProcesses: "العمليات",
      autoRefreshOn: "التحديث التلقائي يعمل",
      autoRefreshOff: "التحديث التلقائي متوقف",
      detailsTitle: "تفاصيل العملية",
      detailsTitleContainer: "تفاصيل الحاوية",
      detailsTitleLock: "تفاصيل قفل الملف",
      detailsAncestry: "السلسلة",
      detailsCwd: "دليل العمل",
      detailsEnv: "البيئة",
      detailsSockets: "المنافذ المفتوحة",
      detailsSource: "المصدر",
      detailsStarted: "وقت البدء",
      detailsCommand: "الأمر",
      detailsUser: "المستخدم",
      detailsNoData: "اختر عملية لعرض التفاصيل",
      detailsLoading: "جارٍ التحميل…",
      detailsNotAvailable: "التفاصيل غير متاحة",
      emptyProcesses: "لا توجد عمليات",
      colCpu: "المعالج",
      colMemory: "الذاكرة",
      colCommand: "الأمر",
      tabContainers: "الحاويات",
      emptyContainers: "لا توجد حاويات",
      colRuntime: "وقت التشغيل",
      colImage: "الصورة",
      colStatus: "الحالة",
      colAction2: "إجراءات",
      actionStop: "إيقاف",
      actionRestart: "إعادة تشغيل",
      actionStart: "تشغيل",
      actionPause: "إيقاف مؤقت",
      actionLogs: "السجلات",
      actionInspect: "فحص",
      tabLocks: "أقفال الملفات",
      emptyLocks: "لا توجد أقفال",
      colFd: "FD",
      colPath: "المسار",
      colInode: "inode",
      locksAllOpen: "عرض جميع الملفات المفتوحة",
      actionTerminate: "إنهاء",
      actionResume: "استئناف",
      actionRenice: "تغيير الأولوية",
      detailsClose: "إغلاق",
      pidPrefix: "pid ",
      containerImage: "الصورة",
      containerState: "الحالة",
      containerCommand: "الأمر",
      containerMounts: "التحميلات",
      containerNetworks: "الشبكات",
      containerEnv: "البيئة (أول 30)",
      actionCopy: "نسخ",
      copied: "تم النسخ",
      copyPort: "نسخ المنفذ",
      copyPid: "نسخ PID",
      copyCommand: "نسخ الأمر",
      copyPath: "نسخ المسار",
      openInBrowser: "فتح في المتصفح",
      actionOpen: "فتح",
      niceValue: "قيمة nice (-20 إلى 19)",
      tabTools: "أدوات",
      mcpMasterLabel: "خادم MCP",
      mcpMasterOn: "مفعّل",
      mcpMasterOff: "معطّل",
      mcpStatusRunning: "نشِط",
      mcpStatusStopped: "متوقّف",
      mcpAutoconfigHint: "مسجّل تلقائيًا مع VS Code و Cursor و Copilot وما إلى ذلك",
      mcpCategoryRead: "قراءة",
      mcpCategoryWrite: "كتابة",
      mcpCategorySystem: "النظام",
      mcpDestructiveFlag: "مدمّر",
      mcpConfigPathHint: "ملف الإعداد",
      mcpServerVersionLabel: "إصدار الخادم",
      mcpSelectAll: "تفعيل الكل",
      mcpDeselectAll: "تعطيل الكل",
    },
  },
  ja: {
    ext: {
      description: "PortPilot",
    },
    cmd: {
      show: "PortPilot: リスニングポートを表示",
      checkPort: "PortPilot: ポートの空き状況を確認",
      killPort: "PortPilot: ポートを終了",
    },
    view: {
      title: "ポート",
    },
    noPorts: "使用中のポートはありません",
    quickPickPlaceholder: "使用中のポート一覧 — 選択してKILL",
    killConfirm: (port, process) => `ポート :${port} (${process}) を終了しますか？`,
    killButton: "KILL",
    inputPromptCheck: "確認するポート番号を入力",
    inputPlaceholderCheck: "例: 3000",
    portFree: (port) => `ポート :${port} は空いています`,
    portUsed: (port, detail) => `ポート :${port} は使用中です${detail}`,
    inputPromptKill: "閉じるポート番号を入力 (カンマ区切りで複数可)",
    inputPlaceholderKill: "例: 3000 または 3000,8080,5432",
    noMatchingPorts: "該当するポートが見つかりません",
    bulkKillConfirm: (n, desc) => `${n} 個のポートを終了: ${desc}`,
    bulkKillResult: (ok, fail) => `完了: 成功 ${ok} / 失敗 ${fail}`,
    killed: (port) => `ポート :${port} を終了しました`,
    killFailed: (msg) => `終了失敗: ${msg}`,
    validateRange: (min, max) => `${min}-${max} の範囲で入力してください`,
    setLanguage: {
      prompt: "言語を選択",
      current: "現在",
      changed: (lang) => `言語: ${lang} (ウィンドウを再読み込み)`,
    },
    webview: {
      searchPlaceholder: "ポート番号 / プロセス名で検索...",
      refresh: "更新",
      bulkKill: (n) => `選択をKILL (${n})`,
      run: "実行",
      statsUsed: (n) => `使用中 ${n}`,
      statsFree: (n) => `空き ${n}`,
      statsTotal: (n) => `合計 ${n}`,
      empty: "該当するポートがありません",
      colPort: "ポート",
      colState: "状態",
      colProcess: "プロセス",
      colPid: "PID",
      colAncestry: "祖先",
      colAction: "操作",
      stateListen: "使用中",
      stateFree: "空き",
      kill: "KILL",
      confirm: "確認",
      cancel: "取消",
      toastKilled: (port) => `:${port} を終了しました`,
      toastKillFailed: (msg) => `終了失敗: ${msg}`,
      bulkKilledLabel: (n) => `${n}個のポート`,
      langMenu: "🌐 言語",
      statsAncestry: "祖先情報あり",
      ancestryNone: "—",
      ancestryLoading: "読み込み中…",
      witrMissing: "祖先情報を取得できません",
      witrPermission: "完全な祖先情報には VS Code を Admin/sudo で起動してください",
      tabPorts: "ポート",
      tabProcesses: "プロセス",
      autoRefreshOn: "自動更新 ON",
      autoRefreshOff: "自動更新 OFF",
      detailsTitle: "プロセスの詳細",
      detailsTitleContainer: "コンテナの詳細",
      detailsTitleLock: "ファイルロックの詳細",
      detailsAncestry: "祖先",
      detailsCwd: "作業ディレクトリ",
      detailsEnv: "環境変数",
      detailsSockets: "ソケット",
      detailsSource: "ソース",
      detailsStarted: "開始時刻",
      detailsCommand: "コマンド",
      detailsUser: "ユーザー",
      detailsNoData: "プロセスを選択して詳細を表示",
      detailsLoading: "読み込み中…",
      detailsNotAvailable: "詳細を取得できません",
      emptyProcesses: "プロセスがありません",
      colCpu: "CPU",
      colMemory: "メモリ",
      colCommand: "コマンド",
      tabContainers: "コンテナ",
      emptyContainers: "コンテナなし",
      colRuntime: "ランタイム",
      colImage: "イメージ",
      colStatus: "ステータス",
      colAction2: "操作",
      actionStop: "停止",
      actionRestart: "再起動",
      actionStart: "開始",
      actionPause: "一時停止",
      actionLogs: "ログ",
      actionInspect: "検査",
      tabLocks: "ファイルロック",
      emptyLocks: "ファイルロックなし",
      colFd: "FD",
      colPath: "パス",
      colInode: "iノード",
      locksAllOpen: "開いているファイルをすべて表示",
      actionTerminate: "終了",
      actionResume: "再開",
      actionRenice: "優先度変更",
      detailsClose: "閉じる",
      pidPrefix: "pid ",
      containerImage: "イメージ",
      containerState: "状態",
      containerCommand: "コマンド",
      containerMounts: "マウント",
      containerNetworks: "ネットワーク",
      containerEnv: "環境変数 (先頭30)",
      actionCopy: "コピー",
      copied: "コピーしました",
      copyPort: "ポートをコピー",
      copyPid: "PIDをコピー",
      copyCommand: "コマンドをコピー",
      copyPath: "パスをコピー",
      openInBrowser: "ブラウザで開く",
      actionOpen: "開く",
      niceValue: "nice値 (-20〜19)",
      tabTools: "ツール",
      mcpMasterLabel: "MCP サーバー",
      mcpMasterOn: "有効",
      mcpMasterOff: "無効",
      mcpStatusRunning: "稼働中",
      mcpStatusStopped: "停止中",
      mcpAutoconfigHint: "VS Code、Cursor、Copilot などに自動登録",
      mcpCategoryRead: "読み取り",
      mcpCategoryWrite: "書き込み",
      mcpCategorySystem: "システム",
      mcpDestructiveFlag: "破壊的",
      mcpConfigPathHint: "設定ファイル",
      mcpServerVersionLabel: "サーバーバージョン",
      mcpSelectAll: "すべて有効",
      mcpDeselectAll: "すべて無効",
    },
  },
};

module.exports = { MESSAGES };
