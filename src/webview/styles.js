...
module.exports = function getStyles() {
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Let cells shrink/truncate instead of forcing horizontal scroll on narrow
     viewports. table-layout:fixed makes column widths predictable; combined
     with max-width + ellipsis on cells it gives a true responsive layout
     without losing the user's resized widths. */
  #mainTable {
    table-layout: fixed;
  }
  #mainTable th,
  ...
  /* Per-column responsive hints: smaller min-widths so the table can fit in a
     narrow sidebar (~300px) without forcing horizontal scroll for every row. */
  #mainTable th[data-col="select"]  { min-width: 32px; max-width: 44px; }
  #mainTable th[data-col="port"],
  #mainTable th[data-col="port2"]   { min-width: 56px; }
  #mainTable th[data-col="state"]   { min-width: 64px; max-width: 90px; }
  #mainTable th[data-col="process"] { min-width: 90px; }
  #mainTable th[data-col="pid"]     { min-width: 56px; max-width: 80px; }
  #mainTable th[data-col="ancestry"],
  #mainTable th[data-col="ancestry2"] { min-width: 110px; }
  #mainTable th[data-col="cpu"]     { min-width: 52px; max-width: 70px; }
  #mainTable th[data-col="memory"]  { min-width: 60px; max-width: 90px; }
  #mainTable th[data-col="cmd"]     { min-width: 110px; }
  #mainTable th[data-col="action"]  { min-width: 70px; max-width: 110px; }
  ...
  * { box-sizing: border-box; }
  ...
  /* ─── Responsive layout ───────────────────────────────────────────
     The webview lives inside the VS Code sidebar, whose width the user can
     drag freely. Make the table degrade gracefully on narrow viewports by
     hiding progressively lower-priority columns and tightening paddings. */

  /* Narrow: hide ancestry/cpu/memory, keep essentials. */
  @media (max-width: 520px) {
    body { font-size: 12px; }
    .sidebar-logo { padding: 6px 10px; gap: 8px; }
    .sidebar-logo-sub { display: none; }
    .toolbar { padding: 8px 10px; gap: 4px; }
    .toolbar input[type="text"] { min-width: 0; padding: 4px 8px; font-size: 11px; }
    .btn { padding: 4px 8px; font-size: 11px; }
    .stats { padding: 6px 10px; gap: 10px; font-size: 11px; }
    thead th, td { padding: 5px 8px; }
    /* Hide long-text columns so the essentials fit. */
    #mainTable th[data-col="ancestry"],
    #mainTable th[data-col="ancestry2"] { display: none !important; }
    #mainTable td.ancestry,
    #mainTable td.col-ancestry { display: none !important; }
    #mainTable th[data-col="cpu"],
    #mainTable th[data-col="memory"] { display: none !important; }
    #mainTable td.cpu,
    #mainTable td.memory { display: none !important; }
  }

  /* Very narrow: also drop the long "cmd" column and shrink action. */
  @media (max-width: 380px) {
    #mainTable th[data-col="cmd"] { display: none !important; }
    #mainTable td.command { display: none !important; }
    .stats .stat-chips { display: none; }
    .kill-btn { padding: 2px 6px; font-size: 10px; }
  }

  /* Make the table cells shrink instead of overflowing when the webview is
     resized between breakpoints. The colgroup widths the user persisted are
     pixel values, so we only relax them when the sidebar becomes narrower
     than the sum of those widths. */
  @media (max-width: 600px) {
    #mainTable { table-layout: fixed; }
    #mainTable col[data-col] { width: auto !important; }
    .process-name { font-size: 11px; }
    .ancestry-chain { font-size: 10px; }
  }
`;
};
